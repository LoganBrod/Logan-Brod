...
These exist in `style/public/` — produced 9 Aug 2026, see `site-assets/RUN-NOTES.md`.

```
/public/closet-building.webm + .mp4 + .jpg   the wardrobe opening — hero, plays once
/public/frames/f_0001.jpg … f_0160.jpg       corridor dolly, 20fps, 1600px, 8.0MB total
/public/bay-{1,2,3,4}.webp                   2560px bay plates: plaster, linen, pale oak, cool plaster
/public/end-wall.webm + .mp4 + .jpg          closing bay, camera arrives and rests
/public/dust.webm + .mp4                     ping-ponged light-and-dust loop, seamless
/public/texture-{1,2,3,4}.webp               waxed cotton, Shetland wool, suede, moleskin
```

The corridor is a frame sequence, not a video element. Preload the frames into an array of
`Image` objects, draw the current one to a `<canvas>` sized with `devicePixelRatio`, and map the
corridor ScrollTrigger's progress onto the frame index. Never call `video.currentTime`. Show
`f_0001.jpg` as a static image until preloading finishes, and skip the sequence entirely below `lg`
and under `prefers-reduced-motion` — show that first frame as a poster instead.
