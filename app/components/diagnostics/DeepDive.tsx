...
export default function DeepDive() {
  ...
  return (
    <section className="mt-10 rounded-2xl border border-roobet-gold/30 bg-roobet-card/50 p-5 sm:p-6">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <h2 className="text-xl font-bold">
          Deep dive: <span className="rank-gold">your own channel</span>
        </h2>
        {state.status === "ready" && (
          <button
            onClick={disconnect}
      ...
      {state.status === "disconnected" && (
        <a
          ...
          className="gold-gradient mt-5 inline-block rounded-xl px-5 py-3 font-semibold text-black"
        >
    ...
    <div className="mt-6 space-y-8">
      <div className="flex items-center gap-3">
        ...
        <a
          ...
          className="font-bold text-white hover:text-roobet-gold"
        >
          {data.channel.name}
        </a>
      </div>
          ...
          <div className="mt-3 space-y-2">
            {data.weekday.map((w) => (
              <div key={w.day} className="flex items-center gap-2 text-xs">
                ...
                <div className="h-4 flex-1 overflow-hidden rounded bg-roobet-border/50">
                  <div
                    className="h-full rounded-r bg-roobet-gold"
          ...
          {videos.map((v) => (
            <li key={v.id} className="text-xs">
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="block truncate font-medium text-gray-200 hover:text-roobet-gold"
                title={v.title}
              >
                {v.title}
              </a>
